...
Four MCP servers put the standards directly inside any AI assistant, as running tools it can call rather than documents to read: evidence-integrity (CROSS, WALKRI, and ORE), CSIS, Dimensional Frame Language, and DS4AI. Each is hosted as a public endpoint and is also published to npm and JSR, so you can connect the hosted server with nothing to install or run it locally. Full connection instructions, every method for every server, are in [Using the MCP servers](using-the-mcp-servers.md).
